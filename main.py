...
#Ncross valid -> risultato = (accuracy:lista, precision:lista, recall:lista)

risultato = Ncross_valid(classification_cnn, X_dataresized, etichette)
average_accuracy = np.mean(risultato[0])
average_precision = np.mean(risultato[1])
average_recall = np.mean(risultato[2])
